...
def solution(recipe, pantry):
    production_count = []
    for name in recipe.keys():
        if not pantry.get(name):   
            return 0
        else:
            production_count.append(pantry[name]//recipe[name])
    total_pos = min(production_count)
    return total_pos


def solution(recipe, pantry):
    return min( [0 if not pantry.get(name) else pantry[name]//recipe[name] for name in recipe.keys()])

def fingerCounting(recipe, ingredients):
    count = 0
    cake =[]
